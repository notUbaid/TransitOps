...
interface StoreValue {
  ...
  updateTrip: (id: string, input: Partial<CreateTripInput>) => Promise<ActionResult<Trip>>;
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const updateTrip = useCallback<StoreValue["updateTrip"]>(
    async (id, input) => {
      if (isApiMode) {
        const json = await apiPost("updateTrip", { id, input });
        if (json.ok) {
          if (json.db) setDb(json.db);
          return { ok: true, data: json.data as Trip };
        }
        return { ok: false, error: json.error ?? "Failed to update trip." };
      }
      const cur = dbRef.current;
